...
## [Unreleased] (Kiro port: shared vault + security + robustness)

### Added

- **Reliability hardening (review I2/I3/I5)**: `wiki-ingest` now has a source size
  guard (warn 5-25 MB, refuse >25 MB or binary); `wiki-autoresearch` rate-limits web
  requests (1-2s between calls, 20-fetch cap, 429 backoff); `wiki-lint` creates a git
  checkpoint commit before any auto-fix so the batch is revertable.
- **Shared-vault layer for Kiro IDE/CLI/Web** (`.kiro/steering/shared-vault.md`). Documents git as the single sync substrate across surfaces: pull-before/push-after locally, branch + pull request from Web (the ephemeral sandbox), per-host transport, and conflict-avoidance rules for `index.md`/`log.md`/`hot.md`.
- **Integrity verification for the installer** (audit S3). `bin/install-kiro.sh` now verifies every downloaded file against a `SHA256SUMS` manifest fetched from the same ref; a mismatch aborts the install. New `--no-verify` and `--strict` flags. Manifest is regenerated by the new `bin/gen-sha256sums.sh`. `shared-vault.md` is now installed by the one-liner too.

### Changed

- **`hot-cache-on-save` hook no longer thrashes during batch work (review I4)**. It now
  skips when a `.vault-meta/batch-in-progress` sentinel exists and debounces if
  `wiki/hot.md` was touched in the last 5 seconds. Batch ingest and lint set/clear the
  sentinel and refresh the hot cache once at the end.
- **Mode-router calls now fail safe (review I6)**. `wiki-ingest`, `wiki-autoresearch`,
  and `wiki-session-harvest` fall back to the generic `wiki/` layout if
  `scripts/wiki-mode.py` is missing or errors, instead of blocking.
- **`wiki-session-harvest` reads Kiro CLI sessions directly (review #5)**. Dropped the
  non-existent `kiro session list`/`export` commands; sessions are discovered and read
  from `~/.kiro/sessions/cli/{id}.json` + `{id}.jsonl` (active `.lock` sessions skipped),
  matching the documented storage layout.
- **`.gitignore` defense-in-depth (review I1)**. Explicitly re-includes `.kiro/` so the
  config stays tracked even under a user's global gitignore.
- **`auto-commit-wiki` hook is now opt-in and safer** (audit S1). It only commits when the gitignored sentinel `.vault-meta/auto-commit.enabled` exists (off by default everywhere, including Web), and no longer auto-commits `.raw/`, so secrets dropped into `.raw/` are never committed without review.
- **`wiki/hot.md` is now gitignored** and untracked. It is a per-host regenerable cache; sharing it caused constant merge conflicts across surfaces.
- **Broader secret redaction in `wiki-session-harvest`** (audit S2). The privacy section now covers database connection strings with embedded passwords, PEM/SSH private-key blocks, JWTs, more API-key prefixes, generic `key=value` secret assignments, and a structured-payload field deny-list (`password`, `token`, `secret`, ...).

### Fixed

- **API keys can no longer leak into logs or the vault** (audit S4, `scripts/contextual-prefix.py`). A new `redact()` helper scrubs the live API key and common secret shapes from any Anthropic-API or `claude` CLI error/stderr text before it is logged.
